...
También está disponible en [Google Classroom](https://classroom.google.com/w/Mzk2OTk1NjE4NzFa/t/all), si desea descargarlo para importarl localmente:
...
```{r, eval=FALSE}
insinstall.packages(c("Rcmdr", "RcmdrMisc", "tidyverse"))
```
...
con el método `levels` usted podrá ver las categorías correspondientes a la variable. Con la variable como factor, puede construir la tabla de frecuencia absoluta con `xtabs`:
...
Los objetos `table` y `xtable` tiene como métodos algunas operaciones matriciales. Por ello, es sencillo construir la tabla de frecuencias relativas (en porcentaje). Por ejemplo, con el objeto `xtable`:
...
`barplot` (de **graphics**) tiene como argumento de entrada una tabla, la cual se puede construir con las variables cualitativas definidas como `factor` o como `character`. Por otro lado, `Barplot` (de **Rcmdr**) recibe como argumento de entrada un vector de clase `factor`. Recuerde, el resultado obtenido con `Barplot` se puede replicar también en la interfaz de RCommander.
...
La respuesta puede complementarse con otros gráficos descriptivos. **Rcmdr** permite graficar histogramas de forma simultánea, de acuerdo con las categorías de una variable cualitativa:
...
Repliquemos este mismo tipo de gráfica con el paquete **tidyverse**
...
En **ggplot2**, cada función que se aplica al _pipe_ tiene argumentos de entrada que modifican el gráfico:
...
A su vez, cada operación que se aplica al _pipe_ va modificando el gráfico. Por ejemplo, `facet_grid` es una función que genera dos gráficos según la variable categórica de agrupación que se especifique:
...
ggplot(data = datos, mapping = aes(x = ESTATURA)) + 
  geom_density() + 
  theme_bw() + facet_grid(. ~ GENERO)
```

Sea con **Rcmdr** o con **ggplot2**, se forzó a ambas categorías de **GENERO** tuvieran los mismos intervalos de clase. Ahora, grafiquemos de manera independiente:
...
par(mfrow = c(1,2))
with(datosM, hist(ESTATURA, main = ""))
with(datosF, hist(ESTATURA, main = ""))
```

¿Son coherentes las gráficas independientes con las simultáneas?

**EJERCICIO** Replique la graficacación simultánea e independiente con la variable **MASA**.
...
_Basado en el procedimiento de la pregunta anterior, responda esta._