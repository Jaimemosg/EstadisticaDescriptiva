...
```{r, eval=FALSE}
install.packages(c("Rcmdr", "RcmdrMisc", "tidyverse"))
```
...
con la función `levels` usted podrá ver las categorías que pertenecen a las variables de tipo `factor`. Con la variable como factor, puede construir la tabla de frecuencia absoluta con `xtabs`:
...
Observe que la tabla es un objeto de clase `xtabs` y `tab`. Con la función `table` también puede construir la tabla de frecuencia absoluta:
...
La tabla es un objeto de clase`tab`. Los objetos `table` y `xtabs` tiene como métodos algunas operaciones matriciales. Por ello, es sencillo construir la tabla de frecuencias relativas (en porcentaje). Por ejemplo, con el objeto `xtable`:
...
_Basado en el procedimiento de la pregunta anterior, responda ésta._